...
#то вычисляется выражение: A-1*AT – K * FТ, иначе вычисляется выражение (A +GТ-F-1)*K,
...
if count < mult:
    ...
    print('\nМатрица F, подматрицы B и C симметрично заменены:\n', F)
else:
    x = A[::,n_second::]
    for i in x:
    ...
    print('\nМатрица F, подматрицы B и C несимметрично заменены:\n', F)
